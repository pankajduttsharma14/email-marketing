        ...
        <div class="col-12">
          <div class="card">
            ...
            <div class="card-body">
              ...
              <div class="create_btn text-right col-md-6 pull-right">
                <button class="btn btn-info waves-effect waves-light" type="button" v-on:click="CreateCampaign">
              ...
              <div class="m-t-40">
                ...
                <v-client-table :data="tableData" :columns="columns" :options="options" ref="table">
                  <div slot="status" slot-scope="{row}">
                    <span v-bind:class="{'label-warning':row.status=='Draft','label-success':row.status=='Completed'}" class="label">{{row.status}}</span>
                  </div>
                  <div slot="actions" slot-scope="{row}">
                    <span class="icon_mat" mdi-edit="" v-on:click="EditCampaign(row.campaign_id)" v-if="row.status=='Draft'">
                                            <i class="mdi mdi-pencil"></i>
                                        </span>
                    <span class="icon_mat" mdi-delete="" v-on:click="Delete(row.campaign_id)">
  ...
  data() {
    return {
      ...
      options: {
        ...
        skin: "display nowrap table table-hover table-striped table-bordered dataTable"
      },
      TemplateHeading: "",
      TemplateText: "",
      TemplateType: "",
  ...
  methods: {
    ...
    Delete(id) {
      ...
      Swal({
        ...
        if (result.value) {
          DeleteCampaign(body).then((response) => {
    ...
    SendMail() {
      ...
      let data = {
        ...
        campaign_id: localStorage.getItem("Temp_id") == "null" ?
    ...
    EditCampaign(id) {
      ...
      ViewCampaign(data).then(res => {
        $('.preloader').css('display', 'none');
        if (res.status == 200) {

          let records = res.data.data[0];
          this.ToggleObjective();
